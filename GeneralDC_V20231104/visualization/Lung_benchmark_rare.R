...
for (method in methods){
  for (output_name in output_names){ # different condition
    temp_files = list.files(paste0(output_name,method),pattern = "*.Rare_0.csv",full.names = T)
    temp_filesname = list.files(paste0(output_name,method),pattern = "*.Rare_0.csv",full.names = F)
...
pdf(file = paste0("Boxplot_RMSE_Pseudo_bulk_Lung_Rare_",Sys.Date(),".pdf"),width = 4,height = 4)
print(p2_rmse_1)
dev.off()
...
write.csv(metric_df,file = paste0("Pseudo_bulk_Lung_Metric_Rare_",Sys.Date(),".csv"))
#----------------------------------------------
# PLOT, ALL
# PCC
...
pdf(file = paste0("Boxplot_PCC_Pseudo_bulk_Lung_Rare_",Sys.Date(),".pdf"),width = 4,height = 4)
print(p2_pcc_1)
dev.off()
